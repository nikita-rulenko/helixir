                    ...
                    .await
                {
                    ...
                    match chain_type {
                        "causal" => {
                            ...
                            for node in &result.because_in {
                                relations.push(ReasoningRelation {
                                    relation_id: format!("rel_{}_{}", &node.memory_id, current_id),
                                    from_memory_id: node.memory_id.clone(),
                                    to_memory_id: current_id.clone(),
                                    relation_type: ReasoningType::Because,
                                    strength: 80,
                                    reasoning_id: None,
                                });
                                next_ids.push(node.memory_id.clone());
                            }
                        }
                        "forward" => {
                            
                            for node in &result.implies_out {
                                relations.push(ReasoningRelation {
                                    relation_id: format!("rel_{}_{}", current_id, &node.memory_id),
                                    from_memory_id: current_id.clone(),
                        ...
                        "both" | "deep" | _ => {
                            ...
                            for node in &result.contradicts_out {
                                relations.push(ReasoningRelation {
                                    relation_id: format!("rel_{}_{}", current_id, &node.memory_id),
                                    from_memory_id: current_id.clone(),
                                    to_memory_id: node.memory_id.clone(),
                                    relation_type: ReasoningType::Contradicts,
                                    strength: 80,
                                    reasoning_id: None,
                                });
                                next_ids.push(node.memory_id.clone());
                            }
                            for node in &result.relation_out {
                                relations.push(ReasoningRelation {
                                    relation_id: format!("rel_{}_{}", current_id, &node.memory_id),
                                    from_memory_id: current_id.clone(),
                                    to_memory_id: node.memory_id.clone(),
                                    relation_type: ReasoningType::Supports,
                                    strength: 80,
                                    reasoning_id: None,
                                });
                                next_ids.push(node.memory_id.clone());
