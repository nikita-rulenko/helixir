    ...
    pub async fn search(
        ...
        let results = match mode.to_lowercase().as_str() {
            ...
            "full" => {
                ...
                debug!("Using hybrid search for full mode (no temporal filter)");
                let hybrid_results = self.hybrid
                    .search(query, Some(user_id), None, limit)
                    .await?;
